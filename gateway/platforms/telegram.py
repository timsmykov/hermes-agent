...
def normalize_telegram_layout(text: str) -> str:
    ...
    for is_code, chunk in _split_fenced_blocks(text):
        if is_code:
            parts.append(chunk)
            continue
        normalized = _wrap_markdown_tables(chunk)
...
class TelegramAdapter(BasePlatformAdapter):
                    ...
                    try:
                        ...
                        if _BadReq and isinstance(send_err, _BadReq):
                            if self._is_thread_not_found_error(send_err) and effective_thread_id is not None:
                                if (
                                    private_dm_topic_send
                                    or self._is_dm_topic_reply_fallback(metadata)
                                    or (metadata and metadata.get("telegram_dm_topic_created_for_send"))
                                ):
                                    return SendResult(
                                        success=False,
                                        error=str(send_err),
                                ...
                                # message still reaches the chat. Private DM
                                # topic sends fail closed above because
                                # dropping the thread leaks into the root lane.
                                logger.warning(
                            ...
                            if "message to be replied not found" in err_lower and reply_to_id is not None:
                                if private_dm_topic_send or self._is_dm_topic_reply_fallback(metadata):
                                    return SendResult(
                                        success=False,
                                        error=str(send_err),
                                        retryable=False,
    ...
    async def _handle_text_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        event.text = self._decorate_forwarded_text(msg, cleaned_text)
    ...
    def _build_message_event(
        ...
        # Store the full replied-to message text/caption, not Telegram's
        # selected partial quote. Telegram users use reply as an explicit
        # context pointer for the next prompt; the agent needs the complete
        # tagged message to avoid losing surrounding instructions/details.
        reply_to_id = None
        reply_to_text = None
